...
out_file = open(r"..\..\verify_endpoints_out.txt", "w", encoding="utf-8")
def log(msg):
...
log("\n=== 4. TESTING /api/v1/pl/departments/summary ===")
res_depts = client.get("/api/v1/pl/departments/summary", headers=headers)
log(f"Status: {res_depts.status_code}")
depts_data = res_depts.json().get("departments", [])
log(f"Departments Count: {len(depts_data)} (Expected: 12)")
for d in depts_data:
    log(f"  {d['department']}: Rev=₹{d['revenue']:,.2f}, Exp=₹{d['expense']:,.2f}, Profit=₹{d['profit']:,.2f}, Margin={d['margin']}%")

log("\n=== 5. TESTING /api/v1/pl/charts ===")
res_charts = client.get("/api/v1/pl/charts?agg=monthly", headers=headers)
log(f"Status: {res_charts.status_code}")
charts = res_charts.json()
log(f"Chart keys: {list(charts.keys())}")
if "trend" in charts:
    trend = charts["trend"]
    log(f"Trend labels count: {len(trend.get('labels', []))}")
    log(f"First label: {trend.get('labels', [])[0]}, Last label: {trend.get('labels', [])[-1]}")
