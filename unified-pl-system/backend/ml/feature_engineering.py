...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 4. amount_zscore
    df['amount_zscore'] = df.groupby('domain')['amount'].transform(lambda x: (x - x.mean()) / x.std()).fillna(0)
    
    # 5. period_rolling_mean (approximation since order matters)
    df = df.sort_values(by=['domain', 'line_item', 'period'])
    df['period_rolling_mean'] = df.groupby(['domain', 'line_item'])['amount'].transform(lambda x: x.rolling(4, min_periods=1).mean())
