...
def get_contamination(domain: str) -> float:
...
def detect_anomalies(records: list) -> list:
    ...
    df = pd.DataFrame([vars(r) for r in records if not r._sa_instance_state.expired])
    if 'id' not in df.columns:
        df['id'] = [r.id for r in records]
        
    df_features = engineer_features(df)
    
    features = ['amount_scaled', 'log_amount', 'domain_encoded', 'line_item_encoded', 
                'year_num', 'period_num', 'amount_zscore', 'period_rolling_mean']
    ...
    for domain, group in df_features.groupby('domain'):
        contam = get_contamination(domain)
    ...
    anomalous_df = final_df[final_df['is_anomaly'] == True]
    if not anomalous_df.empty:
        batch_scores = anomalous_df['anomaly_score'].tolist()
        final_df['percentile_rank'] = final_df.apply(
            lambda row: stats.percentileofscore(batch_scores, row['anomaly_score']) if row['is_anomaly'] else 0.0, 
            axis=1
        )
        final_df['severity'] = final_df.apply(
            lambda row: assign_severity(row['anomaly_score'], batch_scores) if row['is_anomaly'] else 'Low', 
            axis=1
        )
    else:
